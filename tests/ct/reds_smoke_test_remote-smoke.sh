...
#     CASM Test - Cray Inc.
#
#     TEST IDENTIFIER   : reds_smoke_test
#
#     DESCRIPTION       : Automated test for verifying basic REDS API
...
#       various microservices but instead focuses exclusively on the REDS 
...
OUTPUT_FILES_PATH="/tmp/reds_smoke_test_out-${TEST_RUN_TIMESTAMP}.${TEST_RUN_SEED}"
...
function main()
{
    ...
    # GET tests
    for URL_ARGS in \
        "apis/reds/v1/liveness" \
        "apis/reds/v1/readiness"
    do
        URL=$(url "${URL_ARGS}")
        URL_RET=$?
...
{
    run_check_pod_status "cray-reds"
    return $?
}
...
if [[ $? -ne 0 ]] ; then
    ...
    echo "PASS: reds_smoke_test passed!"
    cleanup
    exit 0
fi
